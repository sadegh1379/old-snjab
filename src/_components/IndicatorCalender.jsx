...
class IndicatorCalender extends Component {
        ...
        this.state = {
            ...
            formulaHeaders: [
                {
                    title: 'ردیف',
                    getData: (item, index) => {
                        return  (parseInt(this.state.pageForFormula) - 1) * this.state.per_page + 1 + index
                    },
                    style: {
                        width: '5%'
                    }
                ...
                {
                    title: 'دوره ارزیابی',
                    getData: (item, index) => userActions.getIntervalTitle(this.state.indicator.measure_interval, item.interval_number),
                    style: {
                        width: '15%'
                    }
                },
                {
                    title: 'تاریخ ثبت',
                    getData: (item, index) => moment(item.created_at).format('jYYYY/jMM/jDD'),
                    style: {
                        width: '10%'
                    }
                },
                ...
                {
                    title: ' صورت ' +
                    '<p style="font-size: .7em">(' + props.indicator.numerator + ') </p>',
                    getData: (item, index) => {
                        let res = '';
                        if (item.numerator) {
                ...
                {
                    title: ' مخرج ' + '<p style="font-size: .7em">(' + props.indicator.denumerator + ') </p>',
                    getData: (item, index) => {
                        let res = '';
                        if (item.denumrator) {
            ...
            checklistHeaders: [
                ...
                },
                {
                    title: 'دوره ارزیابی',
                    getData: (item, index) => {
                        return userActions.getIntervalTitle(this.state.indicator.measure_interval, item.interval_number);
                    },
                    style: {
                        width: '15%'
                    }
                ...
                },
                {
                    ...
                    getData: (item, index) => (item.wards.length > 1 ? <Popover
                            ...
                            onClickOutside={() => userActions.toggleState.call(this, 'popOver_open', 'byChecklistRecords', index)}
                        ...
                        >
                            <button
                                onClick={() => userActions.toggleState.call(this, 'popOver_open', 'byChecklistRecords', index)}
                ...
                },
                {
                    title: 'عملیات',
                    getData: (item, index) => (
                        <button className="btn btn-link" onClick={() => this.openDetailRecords(item)} data-tip="جزئیات">
    ...
    componentDidMount() {
        ...
        this.setState({intervals: this.showIntervals(intervals, this.props.measure_interval)})
    ...
    serializer=c=>({...c,_id:c.id,text:c.text,createdAt:c.created_at,user:
            c.user?
                {_id:c.user.id,name:c.user.firstname+' '+c.user.lastname,avatar:userConstants.SERVER_URL+c.user.avatar.url}
                :
   ...
   editFormulaRecord=(interval)=>{
    ...
    deleteFormulaRecord=(id)=>{
        this.props.dispatch(userActions.question("حذف رکورد","آیا از حذف رکورد مورد نظر مطمئن هستید؟")).then(res=>{
            if(res.value){
                this.props.dispatch(userActions.API("delete",`/v2/indicator/answers/formula_records/${id}`)).then(r=>{
                    this.getByFormula();
    ...
    deleteChecklistRecord=(certificate)=>{
        this.props.dispatch(userActions.question("حذف رکورد","آیا از حذف رکورد مورد نظر مطمئن هستید؟")).then(res=>{
            if(res.value){
                this.props.dispatch(userActions.API("delete",`/v2/indicator/answers/checklist_records/detail/${certificate}`)).then(r=>{
                    this.getByChecklist();
    ...
    calc = (data_numerators, data_denumerators) => {
        ...
        if (this.props.indicator.formula.numerator)
            this.props.indicator.formula.numerator.map((num, i) => {
                if ( data_numerators && data_numerators[i] !== null && data_numerators[i] !== undefined) {
        ...
        if (this.props.indicator.formula.denumerator)
            this.props.indicator.formula.denumerator.map((num, i) => {
                if ( data_denumerators && data_denumerators[i] !== null && data_denumerators[i] !== undefined) {
        ...
        if (interval && interval.average !== undefined) {
            ...
            if (lower_limit !== null && upper_limit !== null) {
                if (desirability === 'افزاینده') {
                    return interval.average < lower_limit ? '#bd2130' : (interval.average >= upper_limit ? '#1e7e34' : '#ffc107');
                } else {
                    return interval.average > lower_limit && interval.average < upper_limit ? '#ffc107' : (interval.average <= lower_limit ? '#1e7e34' : '#bd2130');
                }
            } else {
                return '#6c6c6c'
            }
    ...
    showDetailRecod=record=>{
        this.props.dispatch(userActions.API('get',`/v2/indicator/answers/formula_records/${record.id}/answerers`)).then(res=>{
            this.setState({detailAnswerer:true,menuItems:res.data.result})
        })
    }
    getStatus = (interval, index, intervals) => {
        if (index && interval.average !== undefined && intervals[index - 1].average !== undefined) {
            return intervals[index - 1].average > interval.average ? 'icon-font icon-kahesh_shakhes' : 'icon-font icon-afzayesh_shakhes'
        }
        return undefined;
    }
    ...
    async openDetailRecordChecklist (item) {
        ...
        this.props.dispatch(userActions.API('get', `/v2/indicator/answers/checklist_records/detail/records?certificate=${item.certificate}`)).then(res => {
            const questions=[];
            const menuItems=[];
            let ward='';
            res.data.map(q=>{
                const q_i=questions.findIndex(qq=>qq.component===q.component.title);
    ...
    openDetailRecords = (item) => {
        const checklistDetailHeaders = [
            ...
            },
            {
                ...
                getData: (item, index) => item.value !== null ? item.value.toFixed(2) + '%' : '-',
                style: {
                    width: '15%'
                }
            },
            {
                ...
                getData: (item, index) => {
                    return item.value_two_side !== null ? item.value_two_side.toFixed(2) + '%' : '-'
                },
                style: {
                    width: '15%',
                }
    ...
    getDetailRecords = (item, page = 1) => {
        ...
        this.setState({loader}, () => {
            ...
            this.props.dispatch(userActions.API('get', `/v2/indicator/answers/checklist_records/detail?interval_id=${item.interval_id}&page=${page}&per=${this.state.per_page}${q}&ward=${this.state.filterward ? this.state.filterward._id : ""}`, null, false)).then(res => {
                this.setState({
                    loader: undefined,
    ...
    openDetailFormulaModal = (interval_number= null) => {
        ...
        this.getTotal().then(total => {
            const total_answers = total.data.hospital;
            this.setState({detailFormula: true, total_answers}, () => this.getByFormula(1, interval_number))
    ...
    getTotal = () => {
        ...
        return this.props.dispatch(userActions.API('post', `/v2/indicator/count_by_ward_chart?id=${this.state.indicator.id}${q}`, {wards: null}))
    }
    closeChecklist=()=>{
    ...
    onSend=(messages)=>{
        this.props.dispatch(userActions.API('post',`v2/indicator/answers/formula_record_comments`,{
            id:this.state.currentRecord.id,
            text:messages[0].text,
            post:this.props.isCollector?'مسئول اندازه گیری':(this.props.only_me?'مسئول پایش':'دبیر')
        })).then(res=>{
            this.setState({
        ...
        return (
            <>
                <div className="container shadow position-relative">
                    ...
                    <div className="row">
                        <div className="col-lg-3 bg-blue pb-3">
                            {
                                this.props.isIndicator &&
                                <>
                                    <div className="card align-items-center mt-4 py-3">
                                    {this.props.CloseModal && <div className='d-flex justify-content-center mb-4'>
                            <Media query="(max-width:700px)" render={() =>(

                                  <img src={backIcon} alt="backIcon"   onClick={() => this.props.CloseModal()} className="col-7btn d-block mx-auto my-2 px-5" style={{width:'125px',cursor:"pointer"}}/>
                                )}></Media>
                            </div>}
                                        <img src={Gauge} width="60" height="60"/>
                                        <span className="text-center iran-sans_Bold py-2">وضعیت کلی شاخـص</span>

                                        <span
                                            className="iran-sans_Bold text-center py-2">
                                                {this.props.total_average && !isNaN(this.props.total_average) ? userActions.fixed(this.props.total_average) : this.props.total_average}</span>
                                    </div>
                                    <div className="card align-items-center mt-3 pb-4">
                                        <div
                                            className="card-title border-bottom w-75 text-center iran-sans_Bold py-2">راهنمــا
                                            و فیلتـر
                                        </div>
                                       ...
                                       <div className="row" style={{width:"95%"}}>
                                            <button
                                            onClick={() => {
                                                this.selectFilter('#bd2130');
                                            }}
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "fontpwacalender": ""} ${this.props.pwa==="pwastyle"? "": "w-75"} text-center ${!filter || filter === '#bd2130' ? 'bg-danger' : 'bg-dark'}`}>
                                            نیاز به کنترل
                                        </button>
                                        <button
                                            onClick={() => {
                                                this.selectFilter('#ffc107');
                                            }}
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "fontpwacalender": ""} ${this.props.pwa==="pwastyle"? "": "w-75"} text-center ${!filter || filter === '#ffc107' ? 'bg-warning' : 'bg-dark'}`}>
                                            قابل ارتقاء
                                        </button>
                                        <button
                                            onClick={() => {
                                            ...
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "fontpwacalender": ""} ${this.props.pwa==="pwastyle"? "": "w-75"} text-center ${!filter || filter === '#1e7e34' ? 'bg-success' : 'bg-dark'}`}>
                                            مطلوب
                                        </button>
                                       </div>
                                        :
                                        ...
                                        <button
                                            ...
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "w-25": "w-75"} text-center ${!filter || filter === '#bd2130' ? 'bg-danger' : 'bg-dark'}`}>
                                            نیاز به کنترل
                                        </button>
                                        <button
                                            onClick={() => {
                                            ...
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "w-25": "w-75"} text-center ${!filter || filter === '#ffc107' ? 'bg-warning' : 'bg-dark'}`}>
                                            قابل ارتقاء
                                        </button>
                                        <button
                                            onClick={() => {
                                            ...
                                            className={`btn text-white  rounded-pill  py-1 my-2 ${this.props.pwa==="pwastyle"? "w-25": "w-75"} text-center ${!filter || filter === '#1e7e34' ? 'bg-success' : 'bg-dark'}`}>
                                            مطلوب
                                        </button>
                                        </>
}
                                    ...
                                    <div className="text-center mt-4">
                                        <h6 className="iran-sans_Bold text-white">کاربــر گرامــی</h6>
                                        <p className="iran-sans text-center text-justify text-white">برای مشاهده گزارش
                                            آماری این
                                            شاخص <br/> به صفحه
                                            زیر وارد شوید</p>
                                    </div>
                                    <div className="card align-items-center mt-3 border-0 bg-transparent">
                                        ...
                                        <button
                                            ...
                                            className="btn text-blue bg-white rounded-pill  px-4 py-2 my-2 w-100 text-center"
                                        > مشـاهده شناسنامه شاخص
                                        </button>
                                        {this.props.detailFormula && <button
                                        ...
                                        {this.props.detailChecklist &&
                                         ...
                                         <button
                                            ...
                                            className="btn text-blue bg-white rounded-pill  px-4 py-2 my-2 w-100 text-center"
                                        >
                                            {`${this.props.indicator.report_type} های تکمیل شده`}
                        ...
                        <div className="col-lg-9 bg-white">
                            <div className="card-deck mt-4 pb-4 d-flex justify-content-center"
                                 style={{minHeight: '200px'}}>
                                {
                                    this.state.intervals.filter(i => !filter || i.colorTheme === filter).map((c, i) =>
                                        c.component
                                    )
                                }
                            </div>
                ...
                >
                    ...
                    <div className="container-fluid py-5 bg-light">
                        <div className="row d-flex text-right justify-content-center p-3 ">
                            {this.state.detailAnswerer && this.state.menuItems && this.state.menuItems.map((m, i) =>
                                m &&
                                    ...
                                    <label htmlFor="">
                                        {
                                            ...
                                            m.indicator_answerer_info_menu_item?m.indicator_answerer_info_menu_item.menu_item.item:''
                                        }
                                    </label>
                                    {m.indicator_answerer_info_menu_item.menu_item?<div className="pt-2 pb-4 ">
                                        <InputGenerator
                                            readOnly={true}
                                            dispatch={this.props.dispatch}
                                            type={m.indicator_answerer_info_menu_item.menu_item.item_type}
                                            globalStorage={this.props.globalStorage}
                                            placeholder={m.placeholder || m.indicator_answerer_info_menu_item.menu_item.place_holder}
                        ...
                        <div className="row d-flex justify-content-center p-3 ">
                            <button className="btn btn-outline-primary px-5 rounded-pill" onClick={this.closeDetailAnswerer}>بازگشت</button>
                        </div>
                    </div>
                    }
                ...
                >
                    ...
                    <div className="container-fluid py-5 rounded bg-light">
                        ...
                        <HospitalTable
                            ...
                            pageOnChange={(page) => this.getDetailRecords(selectedDetailRecode, page)}
                ...
                >
                    ...
                    <div className="bg-light container pb-5 pt-1 h-100 rounded-lg dirLTR text-right">
                        ...
                        <GiftedChat
                            ...
                            renderAvatar={(item)=>{
                                return(<><div className="overflow_image rounded-circle mx-auto my-2" style={{width:40,height:40}}>
                                    <img src={item.currentMessage.user.avatar} className=""/>
                                </div>
                                    <small style={{fontSize:'.8em'}}>{item.currentMessage.user.name}</small>
                                </>)
                            }}
                            renderAllAvatars={true}
                            showUserAvatar={true}
                            renderDay={itm=>{
                                if(itm.previousMessage && itm.previousMessage.createdAt && moment(itm.currentMessage.createdAt).isSame(moment(itm.previousMessage.createdAt),'d')){
                                    return (<></>)
                                }else{

                                    return (<p className="text-muted text-center" >{moment(itm.currentMessage.createdAt).clone().locale('fa').format('dddd DD MMMM YYYY')}</p>)
                                }
                            }}
                            renderAvatarOnTop
                            renderTime={item=>(<small className="text-white-50 px-2 text-right" style={{fontSize:'.7em'}}>{(item.currentMessage.post || '') +' - '} {moment(item.currentMessage.createdAt).format('HH:mm')}</small>)}
                            placeholder='متن شما ...'
                            messages={[...this.state.comments].reverse()}
                            onSend={(messages) => this.onSend(messages)}
                            user={{
                                _id: this.props.globalStorage.me?this.props.globalStorage.me.id:'',
                            }}
                        />
                    </div>
...
const IndicatorCalenderComp = connect((state) => ({globalStorage: state.globalStorage}))(IndicatorCalender);
