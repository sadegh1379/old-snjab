...
class CheckListCalender extends Component {
    ...
    serializer = c => ({
        ...c, _id: c.id, text: c.text, createdAt: c.created_at, user:
            c.user ?
    ...
    insertValues = (data, interval_number) => {
        this.props.insertValuesModal(data, interval_number)
        ...
        return (
            intervals.map(
                (interval, index) => {
                    ...
                    const colorTheme = interval.average === undefined ? undefined :  "#1e7e34"; // this.getTheme(interval);
    ...
    openDetailRecordsModal = (interval) => {
        this.props.openDetailsModal(interval);
    }
    ...
    getDetailRecords = (item, page = 1) => {
        ...
        this.setState({ loader }, () => {
            ...
            this.props.dispatch(userActions.API('get', `/v2/indicator/answers/checklist_records/detail?interval_id=${item.interval_id}&page=${page}&per=${this.state.per_page}${q}`, null, false)).then(res => {
                this.setState({
                    loader: undefined,
    ...
    openDetailFormulaModal = () => {


        this.getTotal().then(total => {
            const total_answers = total.data.hospital;
            this.setState({ detailFormula: true, total_answers }, () => this.getByFormula())
    ...
    getByFormula = (pageForFormula = 1) => {
        ...
        this.setState({ loader }, () => {
            ...
            this.props.dispatch(userActions.API('get', `/v2/indicator/answers/formula_records?indicator_id=${this.state.indicator.id}&page=${pageForFormula}&per=${this.state.per_page}${q}&ward=${this.state.filterward ? this.state.filterward._id : ''}`, null, false)).then(res => {
                        ...
                        <div className="col-lg-3 bg-blue pb-3">
                                <>
                                    <div className="card align-items-center mt-4 py-3">
                                        ...
                                        <span className="text-center iran-sans_Bold py-2">وضعیت کلی چک لیست</span>
                                    ...
                                    <div className="card align-items-center mt-3 border-0 bg-transparent">
                                        <NavLink
                                            to={`/checklist/dashboard?id=${this.props.checklist_id}&certificate=${this.props.certificate}`}
                                        ...
                                        <button
                                            onClick={this.props.openDoneModal}
                                            className="btn text-blue bg-white rounded-pill  px-4 py-2 my-2 w-100 text-center"
                                        ...
                                        {this.props.detailFormula && <button
                                            onClick={this.openDetailFormulaModal}
                                            className="btn text-blue bg-white rounded-pill  px-4 py-2 my-2 w-100 text-center"
                                        >
                        ...
                        <div className="container-fluid py-5 bg-light">
                            <label className="py-1 mx-auto w-100 d-block" style={{ maxWidth: 350, fontSize: '.8em' }}>
                                <Select className="text-justify custom-select-2"
                                    ...
                                    isClearable
                                    onChange={(v, d) => {
                                        userActions.handleChangeSelect.call(this, v, d, null, null, () => {
                                            this.getByFormula()
                                        })
                                    }}
                                    options={wards}
...
const CheckListCalenderComp = connect((state) => ({ globalStorage: state.globalStorage }))(CheckListCalender);
export { CheckListCalenderComp }
