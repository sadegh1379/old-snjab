...
class IntervalValue_ extends Component {
    ...
    calc = () => {
        ...
        this.props.formula.numerator.map((num, i) => {
            if (num !== '1' && num !== 1 && this.state['numerator_' + i] !== null && this.state['numerator_' + i] !== undefined) {
        ...
        this.props.formula.denumerator.map((num, i) => {
            if (num !== '1' && num !== 1 && this.state['denumerator_' + i] !== null && this.state['denumerator_' + i] !== undefined) {
        ...
        const equal = ((eq_num / (eq_denum || 1)) * this.props.formula.multiplier).toFixed(2);
    ...
    setAnswer = (e, question, answer_index, answer) => {
        ...
        const last_answer = questions[index_array[0]]['content'][index_array[1]]['answer'];
        if (question.question_type === 'سوال باز') {
            ...
            questions[index_array[0]]['content'][index_array[1]]['answer'] = last_answer === answer.answer ? undefined : answer.answer;
            questions[index_array[0]]['content'][index_array[1]]['value'] = last_answer === answer.answer ? undefined : answer_index + 1;
        } else if (question.question_type === 'دوسطحی') {
            questions[index_array[0]]['content'][index_array[1]]['answer'] = last_answer === answer.answer ? undefined : answer.answer;
            questions[index_array[0]]['content'][index_array[1]]['value'] = last_answer === answer.answer ? undefined : answer_index;
        } else {
            questions[index_array[0]]['content'][index_array[1]]['answer'] = last_answer === answer.answer ? undefined : answer.answer
    ...
    componentDidUpdate(prop, state) {
        if (state !== this.state) {
            if (this.props.indicator.report_type !== 'چک لیست' && this.props.indicator.report_type !== 'پرسشنامه') {
                const keys = Object.keys(this.state).filter(k => k.indexOf('numerator_') >= 0);
    ...
    async afterOpenModal() {
        try {
            ...
            let questions = [];
            let checklist_info = {};
            const res_1 = await this.props.dispatch(userActions.API('get', `/v2/get_indicator_answerer_info_menu_items?indicator_id=${this.props.indicator.id}`))
            menuItems = res_1.data.items;
            const res_2 = await this.props.dispatch(userActions.API('get', `/v2/get_indicator_questions?indicator_id=${this.props.indicator.id}`))
            questions = res_2.data.data;
            const res_3 = await this.props.dispatch(userActions.API('get', `/v2/get_indicator_inquire_info?indicator_id=${this.props.indicator.id}`));
    ...
    async submitFormula() {
        ...
        if (isCollector) {
            if (ward) {
                ...
                if (indicator.report_type === 'چک لیست' || indicator.report_type === 'پرسشنامه') {
    ...
    async submitChecklist(e) {
        ...
        if (ward) {
            ...
            this.props.dispatch(userActions.API('post', '/v2/upload_indicator_answers_of_questions', {
                collecting_info:
                {
                    interval_id: interval.id,
    ...
    async wardOnChange(ward) {
        const { numerator_indicator_id, denumerator_indicator_id, numerator_indicator_name, denumerator_indicator_name, } = this.props.formula;
        if (numerator_indicator_id || denumerator_indicator_id) {
            if (numerator_indicator_id && denumerator_indicator_id && numerator_indicator_id === denumerator_indicator_id) {
                ...
                if (interval != null) {
                    ...
                    userActions.failure('ابتدا مقادیر شاخص ' +
                ...
                if (denumerator_indicator_id) {
                    const res = await this.props.dispatch(userActions.API('post', `/v2/indicator/interval_by_ward?id=${denumerator_indicator_id}`, { wards: [ward._id], rotation: ROTATION[this.props.indicator.measure_interval] }));
                    const interval = res.data.wards[0].values.find(v => v.interval_number === this.props.interval.interval_number);
                    if (interval != null) {
                        this.setState({
                            denumerator_0: interval.value.toFixed(2),
                    ...
                    <>
                    {
                        ...
                        ?(
                            <div className="top-nav-error">
                              <img src={WarnIcon} className="top-nav-warn-icon" alt="هشدار" />
                              <p>کاربر گرامی در صورتی که سوالات چک لیست در بخش شما غیر قابل ارزیابی می باشد ، لطفا به آن پاسخ ندهید</p>
                              <img src={WarnIcon} className="top-nav-warn-icon" alt="هشدار" />
                          </div>
                        ):null
                    }
                        ...
                        <div className=" bg-white shadow rounded ">
                            ...
                            <div className="row w-100">
                                ...
                                <div className="col-lg-12 w-100 col-sm-12 p-0 ">
                                    <div className="w-100 bg-blue text-center p-4">
                                        <div className="d-flex align-items-center justify-content-center text-white">
                                            <p style={{ fontSize: '18px' }}>شاخص {indicator.title}</p>
                                            <span style={newStyleforMonthBtn}>{" " + userActions.getIntervalTitle(indicator.measure_interval, interval.interval_number)}</span>

                                        </div>
                                        <div className="form-group w-50 mx-auto  text-center mt-5">
                                    ...
                                    </div>
                                    {
                                        ...
                                        <div className="container-fluid  text-right rounded  py-2 pb-4 bg-white mt-5 ">
                                    ...
                                    <div className="monitor col w-50 mx-auto ">
                                        {isCollector ? (
                                            ...
                                            ''}
                                        <div
                                            className="calculation d-flex flex-column align-items-center py-5 mb-5">
                                            <div className="row justify-content-end w-100"
                                                data-tip={formula.numerator_placeholder}>
                                                <div
                                                    className="rounded-pill flex-wrap shadow-sm text-center d-flex justify-content-center border-0 col-6">
                                                    {formula.numerator.map((itm, i) =>
                                                        <div className="d-flex ml-2" key={i}>
                                                            ...
                                                            />
                                                            <span
                                                                className="iran-sans_Bold text-muted mr-2">{formula.numerator_operators[i]}</span>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="d-flex justify-content-center col-2">
                                                    <Popover
                                                        ...
                                                        padding={10}
                                                        onClickOutside={() => this.setState({ num_help_popover: false })}
                                                        content={({ position, targetRect, popoverRect }) => (
                                                            ...
                                                            >
                                                                <div className="bg-blue rounded p-4">
                                                                    <p className="text-center pb-1 text-white">راهنمای
                                                                        صورت</p>
                                                                    <textarea
                                                                        className={`form-control ${formula.numerator_help.indexOf('\n') >= 0 ? 'rounded' : 'rounded-pill'} border-0 `}
                                                                        name="numerator_help"
                                                                        value={formula.numerator_help}
                                                                        rows={formula.numerator_help.split('\n').length || 1}
                                                    ...
                                                    >
                                                        <button onClick={() => {
                                                            this.setState({ num_help_popover: !num_help_popover })
                                                        }}
                                            ...
                                            <div
                                                ...
                                                <input type="text"
                                                    className="rounded-pill border-0 py-2 px-0 col-2 shadow-sm text-center"
                                                    name="equal"
                                                    readOnly={true}
                                                    value={equal}
                                            ...
                                            <div className="row justify-content-end w-100"
                                                ...
                                                <div
                                                    ...
                                                    {formula.denumerator.map((itm, i) =>
                                                        <div className="d-flex  ml-2" key={i}>
                                                            ...
                                                            />
                                                            <span
                                                                className="iran-sans_Bold text-muted mr-2">{formula.denumerator_operators[i]}</span>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div className="d-flex justify-content-center col-2">
                                                    <Popover
                                                        ...
                                                        padding={10}
                                                        onClickOutside={() => this.setState({ denum_help_popover: false })}
                                                        content={({ position, targetRect, popoverRect }) => (
                                                            <ArrowContainer // if you'd like an arrow, you can import the ArrowContainer!
                                                                position={position}
                                                                targetRect={targetRect}
                                                            ...
                                                            >
                                                                <div className="bg-blue rounded p-4">
                                                                    <p className="text-center pb-1 text-white">راهنمای
                                                                        مخرج</p>
                                                                    <textarea
                                                                        className={`form-control ${formula.denumerator_help.indexOf('\n') >= 0 ? 'rounded' : 'rounded-pill'} border-0 `}
                                                                        name="denumerator_help"
                                                                        value={formula.denumerator_help}
                                                                        rows={formula.denumerator_help.split('\n').length || 1}
                                                                        readOnly={true}
                                                                    />
                                                                </div>
                                                            </ArrowContainer>
                                                    ...
                                                    >
                                                        <button onClick={() => {
                                                            this.setState({ denum_help_popover: !denum_help_popover })
                                                        }}
                                                            className="btn btn-blue rounded-circle d-flex justify-content-center align-items-center"
                                                            style={{ width: 34, height: 34 }}><i
                                                                className="fas fa-question text-white"></i></button>
                                    ...
                                    <div className="w-100 text-right px-3">
                                        <p style={{ fontSize: '18px', fontWeight: 'bold' }} className=" px-3 "> توضیـحات </p>
                                        <div
                                            className=" mx-auto  rounded overflow-hidden w-100 m-4 bg-white"
                                            style={{ height: 200 }}>
                                            <textarea
                                                name="text"
                                                style={{ border: '2px solid #bdbdbd', borderRadius: '5px' }}
                                                onChange={userActions.handleChangeInput.bind(this)}
                                                className="  w-100 h-100 none-outline-focus" value={this.state.text}></textarea>
                                        </div>
                                    </div>
                                ...
                                {/* <div className="col-lg-12  col-sm-12 bg-blue  rounded-right">
                                        <div className="d-flex flex-column align-items-center py-4  ">
                                            ...
                                            <h5 className="lalezar text-center py-1 text-white">
                                                ...
                                                مقادیر شاخص
                                                <span
                                                    className="lalezar ">{" " + userActions.getIntervalTitle(indicator.measure_interval, interval.interval_number)}</span>
                                            </h5>
                                ...
                                <div className="col-lg-12  col-sm-12 bg-blue  rounded-right">
                                    <div className="d-flex flex-column align-items-center py-4  ">
                                        ...
                                        <div className="form-group text-center px-3">
                                            <label
                                                className="description border px-4 rounded-pill bg-white "> توضیحات </label>
                                            <textarea
                                                value={description}
                                                name="description"
                                                onChange={userActions.handleChangeInput.bind(this)}
                                    ...
                                    <div className="monitor col ">
                                        ...
                                        <div className="calculation d-flex flex-column align-items-center py-3">
                                            <div className="row justify-content-end w-100"
                                                data-tip={formula.numerator_placeholder}>
                                                <div
                                                    className="flex-wrap style_pwa_calculation text-center d-flex justify-content-center col-10">
                                                    {formula.numerator.map((itm, i) =>
                                                        <div className="d-flex ml-2" key={i}>
                                                            <input
                                                                type="text"
                            ...
                            <div className="col-lg-4  col-md-6 my-3 position-relative" key={i}>
                                ...
                                <div className="pt-2 pb-4 ">
                                    <InputGenerator
                                        ...
                                        options={m.options ? userActions.generateSelectLabelValue(m.options) : []}
                                        onChange={(data) => {
                                            if (this.props.onChangeInfo) this.props.onChangeInfo(m, data)
                                        }}
                                        value={m.value}
                                    />
                                </div>
    ...
    save = () => {
        ...
        if (selectedUsers.length) {
            const params = {
                ...
                collector: selectedUsers.filter(user => !user.collector_id).map(user => {
                    return { user_id: user.id }
                })
            }
            this.props.dispatch(userActions.API('post', '/v2/add_indicator_collector', params)).then(res => {
                userActions.successToast(res.data.message);
                    ...
                    <div className="container bg-light rounded py-4 shadow-sm">
                        ...
                        <div className="row d-flex justify-content-center mt-3">
                            ...
                            <div className="d-flex flex-column align-items-center mb-3 ">
                                <UsersSelect className="btn animated bounceInUp rounded-pill"
                                    ...
                                    <img data-tip="افــزودن مسئول جمع آوری" src={AddIcon} width="50px"
                                        height="50px" alt="افــزودن مسئول جمع آوری" /></UsersSelect>
                                <ReactTooltip type="light" />
                            </div>
                        </div>
                        <div className="w-100 d-flex justify-content-center">
                            <button type="button" className="btn btn-blue rounded-pill px-5  mx-2"
                                style={{ minWidth: 200 }}
                                onClick={this.handleCloseModal}>انصراف
                            </button>
...
class SelectMonitor_ extends Collector_Monitor {
        ...
        this.state = {
            ...
            headers: [
                ...
                },
                {
                    ...
                    className: 'px-2',
                    getData: (item, index) => {
                        return <Select className="text-justify custom-select-2"
                            value={item.wards}
                            name="wards"
                            placeholder="انتخاب بخش/واحدها"
                            onChange={(value, data) => {
                                userActions.handleChangeSelect.call(this, value, data, 'monitors', index)
                            }}
                ...
                },
                {
                    ...
                    getData: (item, index) => {
                        return (
                            item.monitor_id ?
                                <p className="w-100 text-center m-0">{(item.user_id.fn + ' ' + item.user_id.ln + (item.user_id.post && item.user_id.post !== '-' ? ' (' + item.user_id.post + ')' : ''))}</p>
                                :
                ...
                },
                {
                    ...
                    getData: (item, index) => {
                        return <>
                            <button className="btn btn-link" data-tip="حذف" onClick={() => {
                            ...
                            {!_.isEqual(item.wards_old, item.wards) && item.monitor_id &&
                                <>
                                    <button className="btn btn-link" disabled={item.updating} data-tip="به روزرسانی"
                                        onClick={() => {
                                            this.updateIndicatorMonitor(item, index, this.state.monitors)
                                        }}><i
                                            className={`fal ${item.updating ? ' fa-spinner-third fa-spin text-success' : 'fa-upload text-warning'} `} />
                                    </button>
                                    <ReactTooltip type="light" />
                                </>}
    ...
    getMonitors = () => {
        if (this.props.indicatorId) {
            this.props.dispatch(userActions.API('get', `/v2/get_indicator_monitors?indicator_id=${this.props.indicatorId}`)).then(res => {
    ...
    deleteIndicatorMonitor = (monitor, i) => {
        const { monitors } = this.state;
        this.props.dispatch(userActions.question('حذف مسئول پایش', 'آیا از حذف مسئول پایش مورد نظر مطمئن هستید؟')).then(r => {
            if (r.value) {
                if (monitor.monitor_id) {
                    this.props.dispatch(userActions.API('delete', `/v2/delete_indicator_monitor?monitor_id=${monitor.monitor_id}`)).then(res => {
    ...
    async updateIndicatorMonitor(monitor, i, monitors = null) {
        ...
        const params = {
            ...
            additional_wards: monitor.wards.filter(w => !monitor.wards_old.find(wo => wo._id === w._id)).map(w => w._id),
            removal_wards: monitor.wards_old.filter(wo => !monitor.wards.find(w => wo._id === w._id)).map(w => w._id),
        }
        if (monitors) {
            monitors[i]['updating'] = true;
            this.setState({ monitors });
        }
        const res = await this.props.dispatch(userActions.API('post', `/v2/update_monitor`, params, false));
        if (monitors) {
            userActions.successToast(res.data.message)
    ...
    async addNewMonitors(monitors) {
        ...
        const res = await this.props.dispatch(userActions.API('post', '/v2/add_indicator_monitor', params))
        userActions.successToast(res.data.message);
    ...
    async save() {
        ...
        const err_wards_index = monitors.findIndex(m => !m.wards || !m.wards.length);
        if (err_user_index >= 0) {
        ...
        const oldMonitors = monitors.filter(monitor => monitor.monitor_id && !_.isEqual(monitor.wards_old, monitor.wards));
    ...
    render() {
        return (
            <>
                <button className="btn  rounded-circle" onClick={this.openMonitorModal}><img src={MonitorIcon}
                    width={this.props.iconWidth}
                    height={this.props.iconHeight}
                    alt="monitor" /></button>
                <Modal
                ...
                >
                    ...
                    <div className="container-fluid bg-light rounded py-4 shadow-sm">
                        ...
                        <div className="w-100 d-flex justify-content-center my-3">
                            ...
                            <button type="button" className="btn btn-blue rounded-pill px-5 mx-2"
                                style={{ minWidth: 200 }} onClick={this.save}>ثبت
                            </button>
                        </div>
                    ...
                    <div className="container-fluid row my-3 printIndexForm">
                        <div className="px-5 py-2">
                            <div className="d-flex flex-wrap justify-content-center  text-center">
                                <div className="col-2 border  "><img
                                    src={userConstants.SERVER_URL_2 + hospital.logo.url} width={100} height={95}
                                    className="p-2" /></div>
                                <div
                                    className="col-8 border iran-sans_Bold d-flex align-items-center justify-content-center   "
                                    style={{ fontSize: "1.7em" }}>فـــرم شناسنامــه شــــاخص
                                </div>
                                <div className="col-2 border iran-sans_Bold d-flex align-items-center  "> کــد شاخص :
                                    <span className="px-1">{indicator.code}</span>
                                </div>
                                <div className="col-6 border text-right  iran-sans_Bold  py-3  ">تاریخ تدوین:
                                    <span className="px-1">{indicator.creation_date}</span>
                                </div>
                                <div className="col-6 border text-right  iran-sans_Bold  py-3  "> تاریخ بازنگری:
                                    <span className="px-1">{indicator.edit_date}</span>
                                </div>

                                <div className="col-12 border text-right iran-sans_Bold  py-3  mt-3 "> عنوان شاخص:
                                    <span className="px-1">{indicator.title}</span>
                                </div>
                                <div className="col-12 border text-right iran-sans_Bold  py-3   ">تعریف شاخص و اهمیت
                                    موضوع:
                                    <span className="px-1">{indicator.definition}</span>
                                </div>
                                <div className="col-12 border text-right iran-sans_Bold  py-3   "> دلایل منطقی جمع آوری
                                    شاخص:
                                    <span className="px-1">{indicator.logical_reasons_of_collecting}</span>
                                </div>
                                            ...
                                            {indicator.formula.multiplier &&
                                                <>
                                                    ...
                                                    <div className="mx-4 my-1"
                                                        style={{ backgroundColor: '#ccc', height: 2, flex: '1 1 auto' }} />
                                                </>
                                            }
                                        </div>
                                        <span className="p-1">
                                ...
                                {indicator.formula.numerator_help &&
                                    <div className="col-12 border text-right iran-sans_Bold  py-3   ">
                                        تعریف صورت :
                                        <span className="px-1">{indicator.formula.numerator_help.split('\n').map((d, j) => (
                                            <p className="text-justify" key={j}>{d}</p>))}</span>
                                    </div>}
                                {indicator.formula.denumerator_help &&
                                    <div className="col-12 border text-right iran-sans_Bold  py-3   "> تعریف مخــرج:
                                        <span
                                            className="px-1">{indicator.formula.denumerator_help.split('\n').map((d, j) => (
                                                <p className="text-justify" key={j}>{d}</p>))}</span>

                                    </div>}

                                <div className="col-2 border text-right iran-sans_Bold py-1   "> نوع شاخص

                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "> بعد کیفیت
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "> مبنای تعیین شاخص
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "> اهمیت شاخص از جنبه
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "> دامنه کاربرد(بخش/واحد)
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.indicator_type}</span>
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span
                                        className="px-1">{indicator.quality_dimension.map((q, j) => <span
                                            key={j}> {q} {j + 1 < indicator.quality_dimension.length ? ',' : ''} </span>)}</span>
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.basis}</span>
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.aspect}</span>
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.main_ward_name}</span>
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   ">واحد اندازه گیری
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "> تناوب جمع آوری
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "> منبع گزارش دهی
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "> میزان هدف
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   ">حد شاخص
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.measurement_unit}</span>
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.measure_interval}</span>
                                </div>
                                <div className="col-2 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.source}</span>
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}><span className="px-1">{indicator.target}</span>
                                </div>
                                <div className="col-3 border text-center iran-sans_Bold py-1   "
                                    style={{ minHeight: 100 }}>   <span className="px-1">
                                        <p>حد پایین:
                                            {indicator.lower_limit}</p>
                                        <p>حد بالا:
                                            {indicator.upper_limit}</p>

                                    </span>
                                </div>

                                <div className="col-6 border text-right iran-sans_Bold py-1 h-100  "> مسئول جمع آوری
                                    داده ها
                                </div>
                                <div className="col-6 border text-right iran-sans_Bold py-1 h-100  "> مسئول تحلیل داده
                                    ها
                                </div>
                                <div className="col-6 border text-right iran-sans_Bold py-1 h-100  "><span
                                    className="px-1">{indicator.collectors.map((q, j) => <span
                                        key={j}> {q.firstname + ' ' + q.lastname} {j + 1 < indicator.collectors.length ? ',' : ''} </span>)}</span>
                                </div>
                                <div className="col-6 border text-right iran-sans_Bold py-1 h-100  "><span
                                    className="px-1"> <span className="px-1">{indicator.monitors.map((q, j) => <span
                                        key={j}> {q.user.firstname + ' ' + q.user.lastname} {j + 1 < indicator.monitors.length ? ',' : ''} </span>)}</span></span>
                                </div>

                                <div className="col-12 border text-right iran-sans_Bold  py-3 text-center  ">ارســال
                                    گــزارش پایــش به:
                                </div>
                                <div
                                    className="col-12 border text-right iran-sans_Bold  py-3 d-flex justify-content-around   ">
