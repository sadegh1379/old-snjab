...
export class IndicatorCollectorOrMonitor extends Component {
                ...
                headers: [
                    {
                        ...
                        getData: (item, index) => {
                            return <span
                                className={`${!item.indicator.has_seen ? "iran-sans_Bold" : "iran-sans"
                                    }`}>
                                {index + 1}
                            </span>;
                        },
        ...
        if (indicator.has_seen === false) {
            this.props
                .dispatch(
                    userActions.API(
                        "put",
                        `/v2/burn_indicator_newness_for_${this.state.isCollector ? "collector" : "monitor"
    ...
    getCollectorIndicators = (page = 1, query = this.state.query, filter_by_ward_id ) => {
        this.setState({ query })
        ...
        this.props
            .dispatch(
                userActions.API(
                    "get",
                    `v2/get_collector_indicators?year=${this.props.globalStorage.year}&page=${page}&per=${this.state.per_page}${query || ''}${filter_by_ward_id ? '&filter_by_ward_id=' + filter_by_ward_id : ''}`
                    ,null , false
                    )
            )
            .then(res => {
    ...
    getIndicatorPeriodicity = (id) => {
        this.props.dispatch(userActions.API('get', `v2/get_indicator_interval_averages?indicator_id=${id}&page=${this.state.p_page}&perpage=${10}`, null, true, false))
            .then(res => {
                this.setState({
        ...
        return (
            ...
            <>
                {!this.props.match.params.pwa && <Routes
                    page={isCollector ? "collector" : "monitor"}
                />}

                <div className="container-fluid pb-5">
                    <div className="indicator-RightTitle">
                        <div>
                          <h2  style={{color:'#333' , fontFamily:'iransansBold' , marginRight:'20px'}}>شاخص</h2>
                        </div>
                         <div className="indicator-excelBox">
                            <img src={excelPic} className="indicator-excelIcon" alt="" />
                            <span className="mx-4"> خـروجـی اکـسل </span>
                         </div>
                    </div>
                                    ...
                                    >
                                        {
                                            newWards.map((w, i) => {
                                                return (
                                                    <option key={i}  value={w._id}>{w.name}</option>
                                                )
                                            })
                                        }
                                ...
                                <table className="indicator-tbl">
                                    <thead>
                                        <tr className="indicator-tr" >
                                            <th className="indicator-th" >ردیف</th>
                                            <th className="indicator-th">مبنای شاخص</th>
                                            <th className="indicator-th">عنـوان</th>
                                            <th className="indicator-th">تاریخ ارسال</th>
                                            <th className="indicator-th">دوره تناوب</th>
                                            <th className="indicator-th">واحد متولی</th>
                                            <th className="indicator-th">تارگت</th>
                                            <th className="indicator-th">عملیات</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {
                                            indicators.length ? indicators.map((row, i) => {
                                                const class_name = i % 2 === 0 ? "indicator-td" : "indicator-tdd"
                                                return (
                                                    <tr className="animated fadeIn" key={i} >
                                                        ...
                                                        <td className={class_name}>
                                                             { isCollector ?  row.creation_date : row.indicator.creation_date}
                                                        </td>
                                                        <td className={class_name}>
                                                        ...
                                                        <td className={class_name}>
                                                            <img
                                                                onClick={ev => {
                                                                    this.detailIndicator(isCollector ?  row : row.indicator, i, ev);
                                                                }}
                                                                title="جزییات" style={{ width: '20px', borderRadius: '20px', cursor: 'pointer' }} src={moreIcon} alt="عملیات" />
                                                        </td>
                                                    </tr>
                                                )
                                            }
                                                ...
                                                <tr>
                                                    <td colSpan={8} style={{ textAlign: 'center' }} >
                                                        <p style={{ fontSize: '10px', fontFamily: 'iransansBold' , margin:'10px' }}>
                ...
                >
                    {indicator && (
                        <IndicatorCalenderComp
                            ...
                            OpenMonitorScreenModal={isCollector ? this.OpenMonitorScreenModal : undefined}
                                            ...
                                            <div className="row d-flex justify-content-center  ">
                                                {indicator.creation_date && (
                                                    <div className={"col-lg-3 mt-4 "}>
                                                        ...
                                                        <div
                                                            className="rounded-pill py-2 datediv d-flex justify-content-around align-items-center">
...
const NewIndicatorCollectorOrMonitor = connect(state => ({
    globalStorage: state.globalStorage
}))(IndicatorCollectorOrMonitor);
export { NewIndicatorCollectorOrMonitor };
