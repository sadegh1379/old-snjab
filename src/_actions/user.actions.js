...
<link rel="stylesheet" href="${userConstants.SERVER_URL_2}/snjab_style/assets/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
<link href="${userConstants.SERVER_URL_2}/snjab_style/assets/css/all.min.css" rel="stylesheet" type="text/css">
<style>
@font-face {
  font-family:'IRANSansWeb';
  src:url('${userConstants.SERVER_URL_2}/snjab_style/assets/font/IRANSansWeb.woff') format('woff'),
  url('${userConstants.SERVER_URL_2}/snjab_style/assets/font/IRANSansWeb.ttf') format('ttf') ;
}
@font-face {
  font-family:'iransansBold';
  src:url('${userConstants.SERVER_URL_2}/snjab_style/assets/font/IRANSansWeb_Bold.woff') format('woff'),
  url('${userConstants.SERVER_URL_2}/snjab_style/assets/font/IRANSansWeb_Bold.ttf') format('ttf') ;
}
...
.printIndexForm {
 ...
 print-color-adjust: exact !important;
        ...
        dispatch(userActions.API('post','/v1/pdf',{data: html, name: name.trim()})).then((res)=>{
            window.open(`${userConstants.SERVER_URL}${res.data.url}`,"_blank")
        })
    }
}
function getIndicator(year) {
    return dispatch=>{
        const result=dispatch(userActions.API('get', `/v2/indicator/compact?year=${year}`));
        result.then(res => {
...
function getAvailableInterval(measure_interval) {
    ...
    return generateSelectLabelValue(timelines.splice(timelines.indexOf(measure_interval) + 1, 5));
}
...
function getIntervalTitle(measure_interval,i,year=store.getState().globalStorage.year) {
    return measure_interval==='ماهانه'?persianMonth(i):(measure_interval==='سالانه'?year:(measure_interval==='روزانه'?moment().jDayOfYear(i).format('jMMMM jDD'):
        intervalPerfix(measure_interval)+
        persianNumber(i,true)))
}
function getIntervalTitles(measure_interval,year=store.getState().globalStorage.year) {
    const intervals=[];
...
function handleChangeSelect(value,data,parent,index,callback) {
    if(parent){
        ...
        if(!Array.isArray(arr)){
            ...
            this.setState({ parent: arr},()=>{if(callback)callback(value)});
...
function getWards(hospital) {
    return dispatch=>{
        const res=dispatch(userActions.API('get', hospital?`/v1/search_ward?hospital_id=${hospital}`:'/v1/user/hospital/ward_lists'));
        res.then(r=>{
            dispatch({type:userConstants.WARDS,data:hospital?r.data.result:r.data});
        })
        return res;
    }
}
function Get_all_answerer_info_menu_items() {
    return dispatch=>{
        const res=dispatch(userActions.API('get', '/v2/get_all_answerer_info_menu_items'));
        res.then(r=>{
            dispatch({type:userConstants.ALL_ANSWERER_INFO_MENU_ITEMS,data:r.data.items});
        })
        return res;
    }
}
function getUsers(hospital_id) {
    return dispatch=>{
        const res=dispatch(userActions.API('get', `/v1/search_user?hospital_id=${hospital_id}&query=`,null,false));
        res.then(r=>{
...
function getHospitals(needLoading=true) {
   return dispatch=>{
        dispatch(userActions.API('get', '/v1/hospitals',null,needLoading)).then(res => {
            dispatch(success(res.data));
        })
    }
    function success(data) {
...
function getCommittees(id,year) {
   return dispatch=>{
        dispatch(userActions.API('get', `/v1/list_of_commitees?id=${id}&year=${year}`)).then(res => {
            dispatch(success(res.data));
        })
    }
    function success(data) {
...
function login(params) {
    return dispatch=>{
      const respons= dispatch(API('post', '/v1/user/signin', params,true,false));
        respons.then(res => {
...
function kartabl(year, page, per, menu, submenu) {
    ...
    return dispatch => {
        return dispatch(API('get', `/v1/user/kartabl?year=${year}&page=${page}&per=${per}&menu=${menu}&submenu=${submenu}`))
    }
...
function fixed(num) {
    return !num?num:(
        parseFloat(num.toFixed(2))?num.toFixed(2):(
            parseFloat(num.toFixed(4))?num.toFixed(4):
                num.toFixed(6)
    ));
}
...
function searchItem(query, menu, submenu) {
    return dispatch => {
        return dispatch(API('get', `/v1/user/kartabl/search?query=${query}&menu=${menu}&submenu=${submenu}`))
    }
}

function recentError(year) {
    return dispatch => {
        return dispatch(API('get', `/v1/user/hospital/error_controls/recent?year=${year}`))
    }
}
